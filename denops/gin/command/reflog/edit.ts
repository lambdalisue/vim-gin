...
export async function edit(
  ...
  await exec(denops, bufnr, {
    ...
    ref: unnullish(
      params?.ref,
      (x) => ensure(x, is.String, { message: "ref must be string" }),
    ),
    flags: {
      ...params,
      ref: undefined,
...
export async function exec(
  ...
  await buffer.ensure(denops, bufnr, async () => {
    await batch.batch(denops, async (denops) => {
      ...
      await option.filetype.setLocal(denops, "gin-reflog");
    });
  });
}
...
async function gatherCandidates(
  ...
  const [content, patternStr] = await batch.collect(denops, (denops) => [
    ...
    vars.g.get(denops, "gin_reflog_parse_pattern"),
  ]);
  const pattern = unnullish(
    ...
    (v) =>
      new RegExp(
          ...
          message: "g:gin_reflog_parse_pattern must be string",
        }),
      ),
  );
  const result = parseReflog(content, pattern);
