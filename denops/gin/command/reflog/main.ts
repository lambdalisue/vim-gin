...
async function command(
  ...
  args = await fillCmdArgs(denops, args, "reflog");
  ...
  // GinReflog [{options}]
  // GinReflog [{options}] {ref}
  const ref = residue.at(0);
  if (residue.length > 1) {
    throw new Error("GinReflog accepts at most one positional argument");
  }
