if get(g:, 'gin_branch_supplement_disable', 0) || exists('b:loaded_gin_branch_supplement')
  finish
endif
...
function! s:open_internal(winid) abort
  ...
  try
    ...
    if value ==# get(t:, 'gin_branch_supplement_previous', '')
      return
    endif
    let t:gin_branch_supplement_previous = value
    if value ==# ''
      ...
      silent! call win_execute(
            \ ctx.supplements.log,
            \ printf('Gin! ++buffer log --graph --decorate --oneline %s', value),
            \)
      silent! call win_execute(
